...
**Booste has an endless number of potential use cases.
...
```
We recommend using no earlier than version 0.1.7
...
While filesync is running, any files created, modified, or deleted within this local directory will be automatically uploaded to the codebox. The sync happens in less than 2 seconds for most text-based file saves.
...
Notes on current CLI version:
 ...
 - Filesync starts only when the first one-line-command or "activate" command is ran. It will remain live for 4 hours.
 - If a file is added, modified, or deleted before the filesync process starts, that change will not be reflected. Files sync only when the add/modify/delete event happens while filesync is running. To force-sync files that were missed, we suggest modifying them or moving them out then back into the filesync directory.
...
The codebox will remain running for a time determined by your plan. For beta users, the default is four hours.
